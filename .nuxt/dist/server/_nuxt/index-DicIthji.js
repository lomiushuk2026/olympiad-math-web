...
import { hash } from "/Users/lomiushuk/olympiad-math/web/node_modules/.pnpm/ohash@2.0.11/node_modules/ohash/dist/index.mjs";
import { u as useNuxtApp, a as asyncDataDefaults, c as createError, f as fetchDefaults } from "../server.mjs";
import { debounce } from "/Users/lomiushuk/olympiad-math/web/node_modules/.pnpm/perfect-debounce@2.1.0/node_modules/perfect-debounce/dist/index.mjs";
...
const _sfc_main = /* @__PURE__ */ defineComponent({
  ...
  async setup(__props) {
    ...
    const error = ref("");
    const { data, error: fetchError } = ([__temp, __restore] = withAsyncContext(() => useFetch(
      "/api/grades",
      "$ONsLRVobn_"
      /* nuxt-injected */
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "min-h-screen bg-gray-100" }, _attrs))}><header class="bg-white shadow"><div class="max-w-7xl mx-auto py-6 px-4"><h1 class="text-3xl font-bold text-gray-900">小学奥数题库</h1></div></header><main class="max-w-7xl mx-auto py-6 px-4">`);
      if (loading.value) {
        _push(`<div class="text-center py-10 text-gray-500">加载中...</div>`);
      } else if (error.value) {
        _push(`<div class="text-center py-10 text-red-500">${ssrInterpolate(error.value)}</div>`);
      } else {
        _push(`<div class="grid grid-cols-2 md:grid-cols-3 gap-4"><!--[-->`);
        ssrRenderList(grades.value, (grade) => {
          _push(`<div class="bg-white rounded-lg shadow p-6 hover:shadow-lg cursor-pointer transition-shadow"><div class="text-lg font-semibold text-blue-600">${ssrInterpolate(grade.name)}</div><div class="text-sm text-gray-500 mt-1">${ssrInterpolate(grade.description)}</div><div class="text-xs text-gray-400 mt-2">题目数量: ${ssrInterpolate(grade.questionCount || 0)}</div></div>`);
        });
        _push(`<!--]--></div>`);
      }
      if (selectedGrade.value) {
        _push(`<div class="mt-8"><h2 class="text-xl font-bold mb-4">${ssrInterpolate(selectedGrade.value.name)} - 选择专题</h2><div class="grid grid-cols-2 md:grid-cols-3 gap-3"><!--[-->`);
        ssrRenderList(topics.value, (topic) => {
          _push(`<div class="bg-white rounded-lg shadow p-4 hover:shadow-lg cursor-pointer transition-shadow"><div class="font-medium">${ssrInterpolate(topic.name)}</div><div class="text-xs text-gray-400 mt-1">题目数量: ${ssrInterpolate(topic.questionCount || 0)}</div></div>`);
        });
        _push(`<!--]--></div></div>`);
      } else {
        _push(`<!---->`);
...
//# sourceMappingURL=index-DicIthji.js.map
